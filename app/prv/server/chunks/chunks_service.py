...
from open_ai.extensions.context_filter import ContextFilter
...
class ChunksService:
    ...
    def retrieve_relevant(
        ...
        resp = ""
        for node in nodes:
            # Fetch the entire document instead of just the chunk
            doc_id = node.node.ref_doc_id if node.node.ref_doc_id is not None else "-"
            document = IngestedDoc(
                object="ingest.document",
                doc_id=doc_id,
            ...
            # Create a Chunk with the entire document
            chunk = Chunk(object="context.chunk", score=node.score or 0.0, document=document, text=node.get_content())
            ...
            for n in chunk.previous_texts:
                resp+= " %s"%n
            resp += " %s"%chunk.text
            for n in chunk.next_texts:
                resp+= " %s"%n                
